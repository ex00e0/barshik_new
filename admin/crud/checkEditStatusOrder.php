...
$old_info = mysqli_fetch_assoc(mysqli_query($connect, "SELECT * FROM orders WHERE id_order=$id_prod"));

function check_error ($error) {return "<script> alert('$error'); 
    location.href='../orders.php'; </script>";}                              
$query_update = "UPDATE orders SET ";
$check_update = false; 
if ($old_info["status_order"] != $title) {$query_update .= "status_order = '$title', "; 
                                   $check_update = true;}

if ($check_update) {$query_update = substr($query_update, 0, -2); 
                    $query_update .= " WHERE id_order = $id_prod";
